    ...
    public static unsafe partial class TensorPrimitives
    {
            ...
            where TAggregationOperator : struct, IAggregationOperator<T>
        {
            ...
            if (Vector512.IsHardwareAccelerated && Vector512<T>.IsSupported && TBinaryOperator.Vectorizable)
            {
                ...
                if (remainder >= (uint)Vector512<T>.Count)
                {
                    result = Vectorized512(ref xRef, y, remainder);
                }
                else
                {
                    // We have less than a vector and so we can only handle this as scalar. To do this
                    ...
                    result = VectorizedSmall(ref xRef, y, remainder);
                }
            ...
            if (Vector256.IsHardwareAccelerated && Vector256<T>.IsSupported && TBinaryOperator.Vectorizable)
            {
                ...
                if (remainder >= (uint)Vector256<T>.Count)
                {
                    result = Vectorized256(ref xRef, y, remainder);
                }
                else
                {
                    // We have less than a vector and so we can only handle this as scalar. To do this
            ...
            if (Vector128.IsHardwareAccelerated && Vector128<T>.IsSupported && TBinaryOperator.Vectorizable)
            {
                ...
                if (remainder >= (uint)Vector128<T>.Count)
                {
                    result = Vectorized128(ref xRef, y, remainder);
                }
                else
                {
                    // We have less than a vector and so we can only handle this as scalar. To do this
            ...
            return SoftwareFallback(ref xRef, y, remainder);

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T SoftwareFallback(ref T xRef, T y, nuint length)
            {
                T result = TAggregationOperator.IdentityValue;

                for (nuint i = 0; i < length; i++)
                {
                    result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, i), y));
                }

                return result;
            }

            static T Vectorized128(ref T xRef, T y, nuint remainder)
            {
                Vector128<T> vresult = Vector128.Create(TAggregationOperator.IdentityValue);
                ...
                Vector128<T> beg = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef), yVec);
                Vector128<T> end = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)Vector128<T>.Count), yVec);
                ...
                if (remainder > (uint)(Vector128<T>.Count * 8))
                {
                    ...
                    fixed (T* px = &xRef)
                    {
                        ...
                        while (remainder >= (uint)(Vector128<T>.Count * 8))
                        {
                            ...
                            vector1 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 0)), yVec);
                            vector2 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 1)), yVec);
                            vector3 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 2)), yVec);
                            vector4 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 3)), yVec);
                            ...
                            vector1 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 4)), yVec);
                            vector2 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 5)), yVec);
                            vector3 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 6)), yVec);
                            vector4 = TBinaryOperator.Invoke(Vector128.Load(xPtr + (uint)(Vector128<T>.Count * 7)), yVec);
                ...
                switch (blocks)
                {
                    case 7:
                    {
                        Vector128<T> vector = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)(Vector128<T>.Count * 7)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 6;
                    }
                    ...
                    case 6:
                    {
                        Vector128<T> vector = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)(Vector128<T>.Count * 6)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 5;
                    }
                    ...
                    case 5:
                    {
                        Vector128<T> vector = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)(Vector128<T>.Count * 5)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 4;
                    }
                    ...
                    case 4:
                    {
                        Vector128<T> vector = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)(Vector128<T>.Count * 4)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 3;
                    }
                    ...
                    case 3:
                    {
                        Vector128<T> vector = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)(Vector128<T>.Count * 3)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 2;
                    }
                    ...
                    case 2:
                    {
                        Vector128<T> vector = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)(Vector128<T>.Count * 2)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 1;
                    }
                    ...
                    case 1:
                    {
                        Vector128<T> vector = TBinaryOperator.Invoke(Vector128.LoadUnsafe(ref xRef, remainder - (uint)(Vector128<T>.Count * 1)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 0;
                    }
            ...
            static T Vectorized256(ref T xRef, T y, nuint remainder)
            {
                Vector256<T> vresult = Vector256.Create(TAggregationOperator.IdentityValue);
                ...
                Vector256<T> beg = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef), yVec);
                Vector256<T> end = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)Vector256<T>.Count), yVec);
                ...
                if (remainder > (uint)(Vector256<T>.Count * 8))
                {
                    ...
                    fixed (T* px = &xRef)
                    {
                        ...
                        while (remainder >= (uint)(Vector256<T>.Count * 8))
                        {
                            ...
                            vector1 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 0)), yVec);
                            vector2 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 1)), yVec);
                            vector3 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 2)), yVec);
                            vector4 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 3)), yVec);
                            ...
                            vector1 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 4)), yVec);
                            vector2 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 5)), yVec);
                            vector3 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 6)), yVec);
                            vector4 = TBinaryOperator.Invoke(Vector256.Load(xPtr + (uint)(Vector256<T>.Count * 7)), yVec);
                ...
                switch (blocks)
                {
                    case 7:
                    {
                        Vector256<T> vector = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)(Vector256<T>.Count * 7)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 6;
                    }
                    ...
                    case 6:
                    {
                        Vector256<T> vector = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)(Vector256<T>.Count * 6)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 5;
                    }
                    ...
                    case 5:
                    {
                        Vector256<T> vector = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)(Vector256<T>.Count * 5)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 4;
                    }
                    ...
                    case 4:
                    {
                        Vector256<T> vector = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)(Vector256<T>.Count * 4)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 3;
                    }
                    ...
                    case 3:
                    {
                        Vector256<T> vector = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)(Vector256<T>.Count * 3)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 2;
                    }
                    ...
                    case 2:
                    {
                        Vector256<T> vector = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)(Vector256<T>.Count * 2)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 1;
                    }
                    ...
                    case 1:
                    {
                        Vector256<T> vector = TBinaryOperator.Invoke(Vector256.LoadUnsafe(ref xRef, remainder - (uint)(Vector256<T>.Count * 1)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 0;
                    }
            ...
            static T Vectorized512(ref T xRef, T y, nuint remainder)
            {
                Vector512<T> vresult = Vector512.Create(TAggregationOperator.IdentityValue);
                ...
                Vector512<T> beg = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef), yVec);
                Vector512<T> end = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)Vector512<T>.Count), yVec);
                ...
                if (remainder > (uint)(Vector512<T>.Count * 8))
                {
                    ...
                    fixed (T* px = &xRef)
                    {
                        ...
                        while (remainder >= (uint)(Vector512<T>.Count * 8))
                        {
                            ...
                            vector1 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 0)), yVec);
                            vector2 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 1)), yVec);
                            vector3 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 2)), yVec);
                            vector4 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 3)), yVec);
                            ...
                            vector1 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 4)), yVec);
                            vector2 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 5)), yVec);
                            vector3 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 6)), yVec);
                            vector4 = TBinaryOperator.Invoke(Vector512.Load(xPtr + (uint)(Vector512<T>.Count * 7)), yVec);
                ...
                switch (blocks)
                {
                    case 7:
                    {
                        Vector512<T> vector = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)(Vector512<T>.Count * 7)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 6;
                    }
                    ...
                    case 6:
                    {
                        Vector512<T> vector = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)(Vector512<T>.Count * 6)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 5;
                    }
                    ...
                    case 5:
                    {
                        Vector512<T> vector = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)(Vector512<T>.Count * 5)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 4;
                    }
                    ...
                    case 4:
                    {
                        Vector512<T> vector = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)(Vector512<T>.Count * 4)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 3;
                    }
                    ...
                    case 3:
                    {
                        Vector512<T> vector = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)(Vector512<T>.Count * 3)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 2;
                    }
                    ...
                    case 2:
                    {
                        Vector512<T> vector = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)(Vector512<T>.Count * 2)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 1;
                    }
                    ...
                    case 1:
                    {
                        Vector512<T> vector = TBinaryOperator.Invoke(Vector512.LoadUnsafe(ref xRef, remainder - (uint)(Vector512<T>.Count * 1)), yVec);
                        vresult = TAggregationOperator.Invoke(vresult, vector);
                        goto case 0;
                    }
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall(ref T xRef, T y, nuint remainder)
            {
                if (sizeof(T) == 1)
                {
                    return VectorizedSmall1(ref xRef, y, remainder);
                }
                else if (sizeof(T) == 2)
                {
                    return VectorizedSmall2(ref xRef, y, remainder);
                }
                else if (sizeof(T) == 4)
                {
                    return VectorizedSmall4(ref xRef, y, remainder);
                }
                else
                {
                    Debug.Assert(sizeof(T) == 8);
                    return VectorizedSmall8(ref xRef, y, remainder);
                }
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall1(ref T xRef, T y, nuint remainder)
            {
                Debug.Assert(sizeof(T) == 1);
                T result = TAggregationOperator.IdentityValue;
                ...
                switch (remainder)
                {
                    ...
                    // case to unroll the whole processing.
                    case 15:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 14), y));
                        goto case 14;

                    case 14:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 13), y));
                        goto case 13;

                    case 13:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 12), y));
                        goto case 12;

                    case 12:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 11), y));
                        goto case 11;

                    case 11:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 10), y));
                        goto case 10;

                    case 10:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 9), y));
                        goto case 9;

                    case 9:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 8), y));
                        goto case 8;

                    case 8:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 7), y));
                        goto case 7;

                    case 7:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 6), y));
                        goto case 6;

                    case 6:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 5), y));
                        goto case 5;

                    case 5:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 4), y));
                        goto case 4;

                    case 4:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 3), y));
                        goto case 3;

                    case 3:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 2), y));
                        goto case 2;

                    case 2:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(Unsafe.Add(ref xRef, 1), y));
                        goto case 1;

                    case 1:
                        result = TAggregationOperator.Invoke(result, TBinaryOperator.Invoke(xRef, y));
                        goto case 0;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall2(ref T xRef, T y, nuint remainder)
            {
                Debug.Assert(sizeof(T) == 2);
                T result = TAggregationOperator.IdentityValue;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall4(ref T xRef, T y, nuint remainder)
            {
                Debug.Assert(sizeof(T) == 4);
                T result = TAggregationOperator.IdentityValue;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall8(ref T xRef, T y, nuint remainder)
            {
                Debug.Assert(sizeof(T) == 8);
                T result = TAggregationOperator.IdentityValue;
